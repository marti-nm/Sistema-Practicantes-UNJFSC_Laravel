...
<head>
    ...
    <style type="text/tailwindcss">
        @layer base {
            body {
                @apply font-outfit text-slate-900 bg-white dark:bg-slate-950 dark:text-slate-100 transition-colors duration-300 overflow-x-hidden;
            }
            h1, h2, h3, h4, h5, h6 {
...
>
    ...
    >
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center transition-all">
                <!-- Logo -->
                <a href="/" class="flex items-center gap-2 sm:gap-4 group shrink-0 max-w-[70%] sm:max-w-none">
                    <img src="{{ asset('img/ins-UNJFSC.png') }}" alt="UNJFSC" class="h-9 sm:h-12 w-auto transition-transform group-hover:scale-110">
                    <div class="leading-tight">
                        <p class="text-[13px] sm:text-lg font-extrabold text-secondary dark:text-white tracking-tighter truncate">Sistema de Prácticas</p>
                        <p class="text-[7px] sm:text-[10px] font-bold text-primary uppercase tracking-wider truncate opacity-80">UNJFSC</p>
                    </div>
                </a>
                ...
                <div class="hidden lg:flex items-center gap-8">
                    ...
                    <div class="flex items-center gap-3 px-4 border-l border-slate-200 dark:border-slate-800 ml-4">
                        ...
                        <button 
                            ...
                            class="relative inline-flex h-5 w-10 items-center rounded-full bg-slate-200 dark:bg-slate-700 transition-colors focus:outline-none"
                        >
                            <span 
                        ...
                        <i class="bi bi-moon text-indigo-400 text-sm"></i>
                    </div>
                ...
                <!-- Mobile Toggle/Icons -->
                <div class="lg:hidden flex items-center gap-3 sm:gap-5">
                    <!-- Mobile Theme Toggle -->
                    <button @click="darkMode = !darkMode" class="text-slate-500 dark:text-slate-400 text-xl overflow-hidden focus:outline-none">
                        <i x-show="!darkMode" x-cloak class="bi bi-moon-stars"></i>
                        <i x-show="darkMode" x-cloak class="bi bi-sun"></i>
                    </button>

                    <button @click="mobileMenu = !mobileMenu" class="text-secondary dark:text-white text-2xl focus:outline-none">
        ...
        <div 
            ...
            class="lg:hidden fixed inset-x-0 top-[72px] sm:top-[88px] bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-8 shadow-2xl z-40 max-h-[calc(100vh-80px)] overflow-y-auto"
        >
            <div class="flex flex-col gap-6 text-center">
                <a href="#inicio" @click="mobileMenu = false" class="text-lg font-bold text-slate-700 dark:text-slate-300 hover:text-primary transition-colors">Inicio</a>
                <a href="#nosotros" @click="mobileMenu = false" class="text-lg font-bold text-slate-700 dark:text-slate-300 hover:text-primary transition-colors">Nosotros</a>
                <a href="#roles" @click="mobileMenu = false" class="text-lg font-bold text-slate-700 dark:text-slate-300 hover:text-primary transition-colors">Roles</a>
                <a href="#preguntas" @click="mobileMenu = false" class="text-lg font-bold text-slate-700 dark:text-slate-300 hover:text-primary transition-colors">Ayuda</a>
                <div class="h-px bg-slate-100 dark:bg-slate-800 my-2"></div>
                <a href="{{ route('login') }}" class="btn-access py-4 rounded-xl text-base">Iniciar Sesión</a>
            </div>
        </div>
    </nav>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="flex flex-col lg:flex-row items-center gap-16">
                <div class="lg:w-1/2" data-aos="fade-right">
                    ...
                    <h1 class="text-4xl xs:text-5xl lg:text-7xl font-extrabold text-secondary dark:text-white leading-[1.1] mb-6 tracking-tight">
                        Sistema de <br>
                        <span class="text-primary">Prácticas Pre Profesionales</span>
                    </h1>
                    <p class="text-base sm:text-lg text-slate-600 dark:text-slate-400 mb-8 lg:mb-10 max-w-xl leading-relaxed">
                    ...
                    <div class="flex xs:flex-row gap-4 justify-center lg:justify-start">
                        <a href="#roles" class="px-8 py-4 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark transition-all shadow-xl shadow-primary/20 text-center">
                            Explorar Funcionalidades
                        </a>
                        <a href="{{ route('login') }}" class="px-8 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-secondary dark:text-white font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-center">
                            Iniciar Sesión
                        </a>
                    </div>
                </div>
    ...
    <section id="roles" class="section-padding bg-white dark:bg-slate-950">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div x-data="{ role: 'est' }" class="bg-slate-50 dark:bg-slate-900 rounded-[3rem] p-8 lg:p-12 shadow-inner border border-slate-200/50 dark:border-slate-800/50">
                <!-- Tab Headers -->
                <div class="flex flex-wrap justify-center mb-10 lg:mb-12 gap-2">
                    <div class="bg-white dark:bg-slate-800 p-1.5 sm:p-2 rounded-2xl shadow-sm flex flex-wrap justify-center gap-1 sm:gap-2">
                        <button @click="role = 'est'" :class="role === 'est' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-700'" class="px-4 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold transition-all text-sm sm:text-base">
                            Estudiante
                        </button>
                        <button @click="role = 'doc'" :class="role === 'doc' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-700'" class="px-4 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold transition-all text-sm sm:text-base">
                            Docente
                        </button>
                        <button @click="role = 'emp'" :class="role === 'emp' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-700'" class="px-4 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold transition-all text-sm sm:text-base">
                            Empresa
                        </button>
                    </div>
                </div>
                ...
                <div class="grid lg:grid-cols-1 gap-12 items-center">
                    <div class="space-y-6">
                        <div x-show="role === 'est'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4">
                            <h3 class="text-2xl sm:text-3xl font-extrabold text-secondary dark:text-white mb-6 sm:mb-8 tracking-tight text-center lg:text-left">Servicios para el Estudiante</h3>
    ...
    <section class="section-padding bg-slate-50 dark:bg-slate-900/50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 class="text-3xl sm:text-4xl font-extrabold text-secondary dark:text-white mb-4 tracking-tight">Etapas del Proceso de Prácticas</h2>
            <p class="text-slate-500 mb-10 sm:mb-16">Estructura administrativa oficial de la UNJFSC.</p>
