...
@section('subtitle', 'Administración de facultades académicas')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="{
    ...
    <x-header-content
        ...
        subtitle="Gestión académica oficial de facultades"
        icon="bi-building"
        enableButton="true"
        :typeButton=2
        msj="Registrar Facultad"
        icon_msj="bi-building"
    ...
    <div class="">
        ...
        <div class="overflow-x-auto">
            <table id="tablaFacultades" class="w-full text-left border-collapse table-skeleton-ready">
                <thead>
                    <tr class="bg-gradient-to-r from-primary-dark to-primary text-white">
                        ...
                        <th class="px-6 py-4 text-center text-[11px] font-black uppercase tracking-[0.15em] border-none">Nombre de la Facultad</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900/50">
                    ...
                    <tr class="group hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors duration-200">
                        <td class="px-6 py-4">
                            <span class="text-xs font-bold text-slate-400 dark:text-slate-500">#{{ str_pad($facultad->id, 3, '0', STR_PAD_LEFT) }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="text-sm font-black text-slate-800 dark:text-slate-200 leading-tight tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $facultad->name }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button @click="editModalId = {{ $facultad->id }}" class="p-2.5 rounded-xl bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/40 border border-amber-100 dark:border-amber-800/50 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5" title="Editar">
                                    <i class="bi bi-pencil-square"></i>
                                </button>
                                <button @click="deleteModalId = {{ $facultad->id }}" class="p-2.5 rounded-xl bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/40 border border-rose-100 dark:border-rose-800/50 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5" title="Eliminar">
    ...
    <div x-show="newModal" class="fixed inset-0 z-[1060] flex items-center justify-center px-4" x-cloak>
        ...
        <div x-show="newModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" class="relative bg-white dark:bg-slate-900 rounded-[1rem] shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 dark:border-slate-800">
            ...
            <form action="{{ route('academico.facultad.store') }}" method="POST" class="p-8">
                @csrf
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" name="name" placeholder="Ej: Facultad de Ciencias" class="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-bold text-slate-700 dark:text-slate-200" required>
                        </div>
                        @error('name')
                            <small class="text-rose-500 font-bold ml-1">{{ $message }}</small>
                        @enderror
                    </div>
                </div>
    ...
    @foreach($facultades as $facultad)
    <!-- Modal Editar -->
    <div x-show="editModalId === {{ $facultad->id }}" class="fixed inset-0 z-[1060] flex items-center justify-center px-4" x-cloak>
        <div x-show="editModalId === {{ $facultad->id }}" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="absolute inset-0 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm" @click="editModalId = null"></div>

        <div x-show="editModalId === {{ $facultad->id }}" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" class="relative bg-white dark:bg-slate-900 rounded-[1rem] shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 dark:border-slate-800">
            ...
            <form action="{{ route('academico.facultad.update', $facultad->id) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    <div class="space-y-2 text-left">
                        ...
                        <input type="text" name="name" value="{{ $facultad->name }}" class="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all font-bold text-slate-700 dark:text-slate-200" required>
                    </div>
                </div>
    ...
    <!-- Modal Eliminar -->
    <div x-show="deleteModalId === {{ $facultad->id }}" class="fixed inset-0 z-[1060] flex items-center justify-center px-4" x-cloak>
        <div x-show="deleteModalId === {{ $facultad->id }}" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="absolute inset-0 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm" @click="deleteModalId = null"></div>

        <div x-show="deleteModalId === {{ $facultad->id }}" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" class="relative bg-white dark:bg-slate-900 rounded-[1rem] shadow-2xl w-full max-w-md overflow-hidden border border-slate-100 dark:border-slate-800">
            ...
            <div class="p-10 text-center">
                ...
                <h3 class="text-xl font-black text-slate-800 dark:text-white mb-2 tracking-tight">¿Eliminar esta facultad?</h3>
                <p class="text-slate-500 dark:text-slate-400 text-sm mb-8 leading-relaxed">
                    Estás por eliminar la facultad <strong class="text-rose-600 dark:text-rose-400 font-bold">"{{ $facultad->name }}"</strong>. Esta acción no se puede deshacer y afectará los registros vinculados.
                </p>
                <div class="flex gap-3">
                    ...
                    <form action="{{ route('academico.facultad.destroy', $facultad->id) }}" method="POST" class="flex-[1.5]">
                        @csrf
                        @method('DELETE')
...
$(document).ready(function() {
    $('#tablaFacultades').DataTable({
        ...
        initComplete: function() {
            ...
            $('#tablaFacultades').addClass('dt-ready');
        }
    });
});
</script>
