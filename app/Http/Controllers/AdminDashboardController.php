...
class AdminDashboardController extends Controller
{
    public function indexAdmin(Request $request)
    {
        ...
        $idFacultadSubAdmin =
            $userRolId == 2
                ? $asignacionActual->seccion_academica->facultad->id
                : null;
        ...
        $baseAsignacionQuery = asignacion_persona::where(
            "id_semestre",
            $semestreActivoId,
        );
        ...
        if ($userRolId == 2) {
            $baseAsignacionQuery->whereHas("seccion_academica", function (
                $q,
            ) use ($idFacultadSubAdmin) {
        ...
        $listaEstudiantes = Matricula::whereHas("asignacion_persona", function (
            $query,
        ) use ($baseAsignacionQuery) {
        ...
        $totalEstudiantes = (clone $baseAsignacionQuery)
            ->where("id_rol", 5)
            ->count();
        ...
        $totalMatriculados = Matricula::whereIn(
            "id_ap",
            $asignacionesIds,
        )->count();
        ...
        $totalSupervisores = (clone $baseAsignacionQuery)
            ->where("id_rol", 4)
            ->count();
        ...
        $fichasPorEscuela = Escuela::select(
