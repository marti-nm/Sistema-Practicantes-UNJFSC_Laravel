...
class PersonaController extends Controller
{
    ...
    public function lista_subadmins(Request $request)
    {
        ...
        $query = Persona::whereHas("asignacion_persona", function ($query) use (
            $id_semestre,
            $request,
            $ap_now,
        ) {
            ...
            $query->whereHas("seccion_academica", function ($qsa) use (
                $request,
                $ap_now,
            ) {
        ...
        $personas = $query
            ->with([
                "asignacion_persona" => function ($q) use ($id_semestre) {
                    $q->where("id_semestre", $id_semestre)->where("id_rol", 2);
                },
        ...
        $rutaFilter = "usuario.subadmins";

        return view(
            "list_users.estudiante",
            compact(
                "personas",
                "facultades",
                "escuelas",
    ...
    public function lista_docentes(Request $request)
    {
        ...
        $personas = $query
            ->with([
                "asignacion_persona" => function ($q) use ($id_semestre) {
                    $q->where("id_semestre", $id_semestre)->where("id_rol", 3);
                },
                "asignacion_persona.seccion_academica.facultad",
                "asignacion_persona.seccion_academica.escuela",
                "asignacion_persona.seccion_academica",
    ...
    public function lista_supervisores(Request $request)
    {
        ...
        $personas = $query
            ->with([
                "asignacion_persona" => function ($q) use ($id_semestre) {
                    $q->where("id_semestre", $id_semestre)->where("id_rol", 4);
                },
                "asignacion_persona.seccion_academica.facultad",
                "asignacion_persona.seccion_academica.escuela",
                "asignacion_persona.seccion_academica",
        ...
        $cargo = "Supervisor";
        $rutaFilter = "usuario.dsupervisores";
    ...
    public function lista_estudiantes(Request $request)
    {
        ...
        $personas = $query
            ->with([
                "asignacion_persona" => function ($q) use ($id_semestre) {
                    $q->where("id_semestre", $id_semestre)->where("id_rol", 5);
                },
                "asignacion_persona.seccion_academica.facultad",
                "asignacion_persona.seccion_academica.escuela",
                "asignacion_persona.seccion_academica",
        ...
        $cargo = "Estudiante";
        $rutaFilter = "usuario.estudiantes";
    ...
    public function lista_grupos_estudiantes()
    {
        ...
        $grupo_estudiante = asignacion_persona::with([
            "persona",
            "practicas.empresa",
        ])
            ...
            ->map(function ($ap) {
                ...
                return (object) [
                    ...
                    "empresa" =>
                        $practica && $practica->empresa
                            ? $practica->empresa->razon_social
                            : "Sin Empresa",
                    "etapa" => $practica
                        ? $etapaMapping[$practica->state] ??
                            "Etapa " . $practica->state
                        : "Sin Práctica",
        ...
        return view(
            "list_users.grupo_estudiante",
            compact("grupo", "grupo_estudiante"),
        );
    }
    ...
    public function registro()
    {
        ...
        $rolesQuery = type_users::where("state", 1)->where(
            "name",
            "!=",
            "admin",
        );
        ...
        if ($userRolId == 2) {
            // Sub Admin no puede crear otros Sub Admins
            $rolesQuery->where("name", "!=", "sub admin");
        }

        if ($userRolId == 3) {
            // Docente no puede crear Sub Admins ni otros Docentes
        ...
        $queryFac = Facultad::where("state", 1);
        if ($userRolId == 2 || $userRolId == 3) {
            // Si es Sub Admin o Docente, filtrar por su facultad
        ...
        if ($userRolId == 3) {
            // Si es Docente, filtrar también por su escuela
            $escuelasQuery->where("id", $ap->id_escuela);
        }
        $escuelas = $escuelasQuery->get();

        $semestres = Semestre::where("state", 1)
            ->orderBy("ciclo", "desc")
            ->get();
    ...
    public function getDocentesTitulares($id_sa)
    {
        try {
            $docentes = DB::table("personas")
                ->join(
                    "asignacion_persona as ap",
                    "personas.id",
                    "=",
                    "ap.id_persona",
                )
                ->leftJoin(
                    "grupo_practica as gp",
                    "ap.id",
                    "=",
                    "gp.id_docente",
                )
    ...
    public function verificar($email)
    {
        try {
            ...
            if ($persona) {
                ...
                $asignacionExistente = asignacion_persona::where(
                    "id_persona",
                    $persona->id,
                )
            ...
            return response()->json([
                "error" => "Error al verificar persona: " . $e->getMessage(),
                500,
            ]);
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            if (empty($persona_id)) {
                ...
                $success_message =
                    "Persona creada y asignada al semestre actual correctamente.";
            } else {
                $persona = Persona::find($persona_id);
                $persona_id_final = $persona_id;
                $success_message =
                    "Usuario existente asignado al semestre actual correctamente.";
            }

            $success = $this->asignarPersonaASemestre(
                $persona_id_final,
                $request,
            );
            ...
            return back()->with(
                "error",
                "Ocurrió un error inesperado al registrar. Por favor, intente de nuevo.",
            );
        }
    }
    ...
    private function crearNuevaPersona(Request $request)
    {
        $validatedData = $request->validate([
            ...
            "correo_inst" =>
                "required|email|max:150|unique:personas,correo_inst",
        ...
        $user = User::create([
            ...
            "password" => Hash::make(
                $isStudent ? $request->codigo : "12345678",
            ),
        ]);
    ...
    private function asignarPersonaASemestre($persona_id, Request $request)
    {
        ...
        if ($asignacionExistente) {
            throw new \Exception(
                "Esta persona ya tiene una asignación en este semestre.",
            );
        }

        $rol = $request->rol;
        $is_admin_or_subadmin = in_array($rol, [1]);
        $is_doc_or_sup = in_array($rol, [3, 4, 5]);
    ...
    public function store_masivo(Request $request)
    {
        ...
        try {
            ...
            if ($rol == 5) {
                ...
                $this->formatoRegistroAdministrativo(
                    $request,
                    $rol,
                    $id_semestre,
                );
            ...
            return back()->with(
                "success",
                "Formulario de Trámite (FUT) subido correctamente.",
            );
        } catch (\Exception $e) {
            return response()->json(
                [
    ...
    public function formatoRegistroAdministrativo($request, $rol, $id_semestre)
    {
        ...
        foreach ($contenido as $linea) {
            ...
            if (count($datos) !== count($headers)) {
                $errores[] =
                    "Formato incorrecto en la línea " . ($usuariosCreados + 1);
                continue;
            }
            ...
            foreach ($headers as $index => $header) {
                if (isset($campoMap[$header])) {
                    if ($header === "persona") {
                        ...
                        $usuarioData["apellidos"] = implode(
                            " ",
                            array_slice($partes, 0, 2),
                        );
                        $usuarioData["nombres"] = implode(
                            " ",
            ...
            try {
                ...
                $errores[] =
                    "Error al crear usuario en la línea " .
                    ($usuariosCreados + 1) .
                    ": " .
                    $e->getMessage();
            }
    ...
    public function createPersonaMasivo(
        $usuarioData,
        $rol,
        $id_semestre,
        $seccion,
        ...
        if (!$user) {
            $user = User::create([
                ...
                "password" => Hash::make(
                    $rol != 5 ? "12345678" : $usuarioData["codigo"],
                ),
            ]);
        ...
        if (!$asignacion) {
            ...
            Log::info(
                "Asignación creada correctamente para: " . $persona->nombres,
            );
        } else {
        ...
        try {
            ...
            if ($request->hasFile("ruta_foto")) {
                $nombre =
                    ...
                    time() .
                    "." .
                    $request->file("ruta_foto")->getClientOriginalExtension();
                $ruta = $request
                    ->file("ruta_foto")
                    ->storeAs("fotos", $nombre, "public");
    ...
    public function disabledAsignacion(Request $request)
    {
        ...
        return back()->with(
            "success",
            "Asignación " . $opcion . " correctamente.",
        );
    }
    ...
    {
        $solicitud_baja = solicitud_baja::where("id_ap_delete", $id_ap)
            ->where("id_sa", $id_sa)
            ->latest()
            ->first();
    ...
    public function solicitudBaja(Request $request)
    {
        ...
        $solicitud_baja = solicitud_baja::where(
            "id",
            $request->id_sol,
        )->first();
        $solicitud_baja->estado_sol =
            $request->estado == 1 ? "aceptado" : "rechazado";
        $solicitud_baja->comentario_admin = $request->comentario;
        $solicitud_baja->save();

        if ($request->estado == 1) {
            $ap = asignacion_persona::where(
                "id",
                $solicitud_baja->id_ap_delete,
            )->first();
            if ($solicitud_baja->tipo_sol == "deshabilitar") {
                $ap->state = 4;
                $ap->save();
            } elseif ($solicitud_baja->tipo_sol == "eliminar") {
                // Verificamos si la persona tiene otras asignaciones
                $persona = $ap->persona;
                $asignacionesCount = asignacion_persona::where(
                    "id_persona",
                    $persona->id,
                )->count();
            ...
            } elseif ($solicitud_baja->tipo_sol == "habilitar") {
                $ap->state = 1;
                $ap->save();
            }
        }
        ...
        return back()->with(
            "success",
            "Solicitud de baja enviada correctamente.",
        );
    }
    ...
    public function getPersonaForEdit($id)
    {
        ...
        $persona = Persona::with([
            "asignacion_persona" => function ($q) use ($id_semestre) {
                $q->where("id_semestre", $id_semestre)->with([
                    "seccion_academica.facultad",
                    "seccion_academica.escuela",
                ]);
            },
